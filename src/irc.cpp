...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("Levopluscoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LevopluscoinTEST2\r");
            Send(hSocket, "WHO #LevopluscoinTEST2\r");
        } else {
            // randomly join #Levopluscoin00-#Levopluscoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Levopluscoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Levopluscoin%02d\r", channel_number).c_str());
        }
